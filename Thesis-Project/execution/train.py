...
sweep_config = {
    # search strategy
    "method": "random",
    "metric": {"name": "mean_f1_score", "goal": "maximize"},
    "parameters": {
        "epoch": {"value": 2},
        "learning_rate": {
            "min": 0.001,
...
def train():
    ...
    hf_transforms = VideoTransform(
        dataset="HockeyFights", json_file="augmentation_values.json"
    )
    ...
    for dataset in [hf_dataset]:
        ...
        for n, fold in enumerate(dataset.k_fold(n_folds=wandb.config.n_folds)):
            ...
            criterion = BCELoss()
            optimizer = Adam(
                params=vd_model.parameters(), lr=wandb.config.learning_rate
            )
            scheduler = StepLR(
                optimizer=optimizer,
                step_size=wandb.config.step_size,
                gamma=wandb.config.factor,
            )
            ...
            for epoch in range(wandb.config.epoch):
                ...
                for batch in dataloader:
                    ...
                    losses_per_batch = np.append(
                        losses_per_batch, loss.detach().cpu().item()
                    )
                ...
                all_outputs, all_labels = (
                    np.array(cumulating_outputs) >= wandb.config.treshold
                ), np.array(cumulating_labels)
                f1_score_result = f1_score(
                    y_true=all_labels, y_pred=all_outputs, average="binary"
                )
                ...
                wandb.log(
                    {
                        wandb_name_log + "_val_loss": avg_loss,
                        wandb_name_log + "_f1score": f1_score_result,
                        "epoch": epoch,
                    }
                )

            # plot batch losses for each epoch and save to wandb
            image_buffer = plot_to_image_buff(
                x=losses_per_batch, label="Loss per batch"
            )
